...
export default function ThemeToggle({ value, onChange }) {
  ...
  return (
    <button
      ...
      className="group flex items-center rounded-2xl px-2 py-1 bg-slate-800/60 ring-1 ring-white/15 shadow-inner transition-all hover:ring-white/25"
    >
      <Sun className={`h-4 w-4 mr-1 ${value ? "text-slate-400 opacity-40" : "text-amber-300"}`} />
      <span className={`relative h-6 w-12 rounded-full border border-white/15 ${value ? "bg-indigo-500/70" : "bg-slate-700/70"}`}>
